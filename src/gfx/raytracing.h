...
public:
    ...
    // refitLimit 이 0 이 아니면(클러스터 모드의 부분 재구축 프레임: 오브젝트 구성은 그대로, 변환·포즈만 바뀜) 인스턴스
    // 수가 지난 구축과 같고 이어진 갱신이 그 수에 못 미칠 때 새로 세우지 않고 제자리에서 갱신(refit)한다. 클러스터
    // 모드의 상위 구조는 그러려고 ALLOW_UPDATE 로 세운다.
...
private:
    ...
    // 마지막 전체 구축 뒤 이어진 refit 수. 상한은 RenderSettings::clusterRefitLimit 이 정한다.
    uint32_t topLevelRefits = 0;
